...
(**[backtrack t spaceR spaceC tracking] takes in a generator.t object, 
   row and column of current space, and a boolean to tell us if we are 
   currently tracking.  The algorithm adds valid values back to the row, column and 
   box values for t.  It also updates resDic and tried to tell us which values
   we have tried that have failed.  The algorithm is used to track back until we 
   find a valid approach to the sudoku board.
   Returns: updated t object*)
...
(**[board_helper spaceR spaceC t] takes in the row and column of the current
   space and the Generator.t object.  It does the work for create_board and 
   returns a completed, valid Generator.t object.*)
